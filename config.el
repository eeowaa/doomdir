...
(defun cli-help--get-buffer-create (cmdline)
  "Return a buffer for displaying CMDLINE help text, creating a new one if needed.
`cli-help-mode' is enabled in new buffers."
...
(defun cli-help-fontify-buffer ()
  ...
  (interactive)
  (if-let ((program (car-safe (split-string cli-help--command-line)))
           (fontifier (cdr (assoc program cli-help-fontifier-alist))))
      (funcall fontifier)
    (funcall cli-help-fontifier-function)))
    ...
    (while (not (eobp))
      (cond
       ((looking-at "^\\(?:Group\\|Command\\)$")
        (put-text-property (bol) (eol) 'font-lock-face 'cli-help-heading)
        (forward-line)
          ...
          (cond
           ...
           ((while (looking-at example-re)
              ...
              (when-let ((beg (and (looking-at ".*-$") (match-end 0)))
                ...
                (put-text-property beg (eol) 'font-lock-face example-face))
              (forward-line)))
           (t (forward-line))))
        (forward-line -1))
    ...
    (setq-local outline-level (lambda () 1)
                ...
                (format "\\(?:%s\\)"
                        (string-join '("Group" "Subgroups:" "Commands:"
                                       "Command" "Arguments" "Global Arguments" "Examples")
                                     "\\|")))
    (outline-minor-mode))
  (redisplay))
...
(defun cli-help (cmdline)
  ...
  (interactive "sCommand line: ") ;; TODO: Default to `cli-help--command-line'
  (unless (string-match-p "\\S-" cmdline)
    (user-error "Command line must not be empty"))
  (let* ((fields (split-string cmdline))
         (program (car fields))
         (args (cdr fields))
         (help-flag (if-let* ((entry (assoc program cli-help-flag-alist)))
                        (cdr entry)
                      cli-help-default-flag))
         (buffer (cli-help--get-buffer-create cmdline)))
    (pop-to-buffer buffer)
...
;; TODO Add a function to select another buffer in `cli-help--buffers'
;; TODO Integrate with `vterm' (preferably with a keybinding)
;; TODO Add `evil' bindings
;; TODO Start with `hl-line-mode' in `cli-help-mode'
