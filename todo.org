...
*UPDATE*: A better alternative may be to maintain a personal AppImage containing
Emacs, Doom, and all packages. This way, I can always be sure to have the exact
same Emacs (sans dotfiles and external program dependencies) on all Linux
machines that I work on. Here is a good reference: https://github.com/blahgeek/emacs-appimage

** TODO [#A] Start transitioning to built-in packages :tweak:
Although external packages can sometimes provide more/better functionality than
built-in packages, it often takes considerably more integration work to get them
to play nicely together. When an external package is needed, it is better to use
a package that integrates tightly with built-in packages.

+ projectile :: project
+ flycheck :: flymake
+ lsp-mode :: eglot
+ dap-mode :: [[https://github.com/svaante/dape][dape]] (external package)
+ treemacs :: [[https://github.com/alexluigit/dirvish][dirvish]] (external package)
+ company :: [[https://github.com/minad/corfu][corfu]] (external package)
+ vterm :: [[https://github.com/kephale/emacs-eat][eat]] (external package)
...
** OKAY [#C] Make ~PREREQ~ lines OS-independent
:LOGBOOK:
...
** OKAY [#C] Normalize curl calls in prerequisite installation
...
*UPDATE*: For now, I just always pass ~-fsSL[oO]~ to ~curl~ (using the mnemonic
"f-ing sssslow") and sometimes pass additional flags that I might not need.
Here is what each of my chosen-standard flags mean:

#+begin_example
-f, --fail           Fail silently on HTTP 5xx response.
-s, --silent         Don't show the progress meter or error messages.
-S, --show-error     Show an error message if curl fails.
-L, --location       Automatically redo the request upon HTTP 3xx response.
-o, --output <file>  Write output to <file> instead of stdout.
-O, --remote-name    Write output to a local file named after the remote.
#+end_example

** STRT [#A] Add tags to all literal config subtrees
+ Ignore ~Install prerequisites~ subtrees
+ Primary tags (must have exactly one) are as follows:
  - ~feat~  :: New feature
  - ~bug~   :: Bug fix
  - ~tweak~ :: Configuration preference
  - ~doc~   :: ???
    + Often used with the ~noexport~ tag
    + Contrast with the ~COMMENT~ TODO keyword
+ Secondary tags (in addition to primary tags) are as follows:
  - ~hack~   :: Kludge or fragile workaround
    + Add this tag when advising functions or macros
    + Periodically revisit these entries
  - ~module~ :: Custom module
    + Should be extracted to =modules/<category>/<module>/=

**TODO:** Find all TODO items without a ~feat~, ~bug~, ~tweak~, or ~doc~ tag. \\
Reference: [[info:org#Matching tags and properties][org#Matching tags and properties]]

** TODO [#A] Create custom Doom modules according to ~:module:~ tags
To get an Org Agenda view containing all subtrees in =config.org= that have a
~:module:~ tag:

1. Open a buffer to =config.org= and make it current
2. Run ~org-agenda-set-restriction-lock~ with a prefix argument (=SPC u C-c C-x <=)
   to restrict the Org Agenda to the current buffer's file
3. Run ~org-tags-view~ (=SPC o a m=) and enter ~module~ when prompted to open an
   Org Agenda view with all subtrees with the ~:module:~ tag (direct or inherited)
4. Run ~org-agenda-remove-restriction-lock~ (=C-c C-x >=) when finished to remove
   the Org Agenda file restriction

The following code block effectively does the same thing, except that tag
inheritance is ignored for whatever reason (see also: ~org-use-tag-ineritance~,
~org-agenda-use-tag-inheritance~):

#+begin_src emacs-lisp :tangle no
(let ((org-agenda-files (list (expand-file-name "config.org" doom-user-dir))))
  (org-tags-view nil "module"))
#+end_src
