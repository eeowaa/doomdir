...
this config (currently only macOS). That said, all you need to do is run The
[[file:install/run][install/run]] script, which is a wrapper that will automatically dispatch the
appropriate system-specific script.
